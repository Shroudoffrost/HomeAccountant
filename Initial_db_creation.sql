...
create database HomeAccountant
go
...
create table t_Stores (
	...
	constraint FK_Store_name foreign key (Name) references t_Sell_point_names (ID),
	constraint FK_Store_address foreign key (Address) references t_Addresses (ID)
);
...
create table t_Products (
	...
	constraint FK_Category_ID foreign key (Category) references t_Categories (ID),
	constraint FK_Receipt_ID foreign key (receipt) references t_Receipts (ID)
);